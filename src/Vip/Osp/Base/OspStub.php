...
namespace LeagueAndShop\EasyTBK\Vip\Osp\Base;

//use Thrift\Transport\TFramedTransport;
//use Thrift\Type\TMessageType;
//use Thrift\Transport\TSocket;
//use Thrift\Transport\TBufferedTransport;
use LeagueAndShop\EasyTBK\Vip\Osp\Context\InvocationContextFactory;
...
class OspStub
{
    ...
    {
        \LeagueAndShop\EasyTBK\Vip\Osp\Log\Logger::info("[OspStub][InitInvocation][Service-Version-Method][" . $this->serviceName . "-" . $this->version . "-" . $method . "]");
        ...
        , $timestamp, $version, $request, $appSecret)
    {
        ...
        return \LeagueAndShop\EasyTBK\Vip\Osp\Util\HmacUtil::hmac($sign, $appSecret);
    }
    ...
    {
        if (!$restAddr) throw new \LeagueAndShop\EasyTBK\Vip\Osp\Exception\OspException("The openapi appURL is null, please set the appURL value");
    ...
    {
        $ctx = InvocationContextFactory::getInstance();
        $oprot = new \LeagueAndShop\EasyTBK\Vip\Osp\Protocol\JSONProtocol(new \LeagueAndShop\EasyTBK\Vip\Osp\Buffer\MemoryBuffer());
        $args->write($oprot);
        ...
        $client = new \LeagueAndShop\EasyTBK\Vip\Osp\Http\HttpClient($scheme, $host, $port);
        $request = array();

        $appKey = $ctx->getAppKey();
        if ($appKey == null) throw new \LeagueAndShop\EasyTBK\Vip\Osp\Exception\OspException("The openapi appKey is null, please set the appKey value");

        $appSecret = $ctx->getAppSecret();
        if ($appSecret == null) throw new \LeagueAndShop\EasyTBK\Vip\Osp\Exception\OspException("The openapi appSecret is null, please set the appSecret value");
        ...
        $timestamp = round(\LeagueAndShop\EasyTBK\Vip\Osp\Util\TimeUtil::currentTimeMillis() / 1000);
        ...
        try {
            ...
            throw new \LeagueAndShop\EasyTBK\Vip\Osp\Exception\OspException(\Osp\Util\MessageUtil::getInvocationMsg($ctx, $e->getMessage()), $e);
        }
    ...
    public function receive_base($result)
    {
        ...
        if (\LeagueAndShop\EasyTBK\Vip\Osp\Base\OspStub::$RETURN_NULL != $response) {
            $iprot = new \LeagueAndShop\EasyTBK\Vip\Osp\Protocol\JSONProtocol(new \LeagueAndShop\EasyTBK\Vip\Osp\Buffer\MemoryBuffer());
            ...
            if (!"0" == $returnCode) {
                ...
                throw new \LeagueAndShop\EasyTBK\Vip\Osp\Exception\OspException($returnMessage, $returnCode);
            }
